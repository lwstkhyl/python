...
create table canteen(
    address varchar(32),
    canteen_name varchar(32) unique, -- 食堂名唯一
    food_count int
);
create table food(
...
insert into food values("炒饭",15,"咸辣","二食堂");
update canteen set food_count = food_count + 1 where canteen_name = "二食堂";
insert into food values("面",20,"甜","二食堂");
update canteen set food_count = food_count + 1 where canteen_name = "二食堂";
insert into food values("盖浇饭",17,"咸","三食堂");
...
-- 根据食堂查询菜品信息
select * from food where canteen_name = "二食堂" order by price desc;
-- 查询所有食堂的盖浇饭的全部信息
...
-- 查询所有食堂的口味为辣的全部信息
select canteen.address,canteen.canteen_name,food.food_name,food.price,food.taste 
from canteen,food where food.taste like "%辣%" and 
food.canteen_name = canteen.canteen_name;
-- 查询价格在[10,17]的菜品
select canteen.address,canteen.canteen_name,food.food_name,food.price,food.taste 
from canteen,food where food.price between 10 and 17 and 
food.canteen_name = canteen.canteen_name;
...
通常用于一张图中画多条线的情况
`图对象.legend([标签1, 标签2, ...])`
- 图对象可以是`plt`，也可以前面提过的子图对象`ax`，以下都以`plt`表示图对象。如果使用子图对象`ax`时报错，可将函数写成`set_xxx`的形式（如`set_title`）
...
相关概念：
- **上四分位数**又称Q~3~，表示数据从小到大的3/4位置
- **下四分位数**又称Q~1~，表示数据从小到大的1/4位置
- **四分位距**(IQR)：Q~3~-Q~1~
...
plt.show()
```
![matplotlib绘图68](./md-image/matplotlib绘图68.png){:width=300 height=300}
...
plt.show()
```
![matplotlib绘图69](./md-image/matplotlib绘图69.png){:width=300 height=300}
**任务3：用柱形图标识在每一节的出手次数**
...
plt.show()
```
![matplotlib绘图74](./md-image/matplotlib绘图74.png){:width=300 height=300}
...
plt.show()
```
![matplotlib绘图70](./md-image/matplotlib绘图70.png){:width=300 height=300}
**任务5：标识投篮位置的区域**
即在第一个图（投篮位置坐标）的基础上，用颜色标识每个点都属于哪个区域，同区域的点为同一种颜色，例如：
![matplotlib绘图71](./md-image/matplotlib绘图71.png){:width=300 height=300}
...
plt.show()
```
![matplotlib绘图72](./md-image/matplotlib绘图72.png){:width=300 height=300}
...
plt.show()
```
![matplotlib绘图73](./md-image/matplotlib绘图73.png){:width=300 height=300}

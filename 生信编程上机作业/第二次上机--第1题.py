...
def set_correlation(n:int):
    ...
    return res
# 基因关联网络构建
nodes = transcriptome_data.loc[:, 't-Cont1':'t-KD6']  # 选取结点：'t-Cont1'到't-KD6'
G = nx.Graph()  # 创建网络图
G.add_nodes_from(nodes.columns)  # 添加节点
correlation_matrix = set_correlation(len(nodes.columns))  # 此处应为基因间的相关性数据
threshold = 3  # 设置阈值，可以根据阈值选择相关性大于某个值的边
for i in range(len(correlation_matrix)):
    for j in range(i + 1, len(correlation_matrix)):
        if correlation_matrix[i][j] > threshold:
            G.add_edge(transcriptome_data.columns[i], transcriptome_data.columns[j])  # 添加边
